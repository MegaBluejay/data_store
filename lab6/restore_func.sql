...
create or replace function restore(branch branches)
...
begin
    ...
    where
        si.branch_id = branch.id
        and
        lsi.deleted_at is null
    order by si.local_id;

    perform public.dblink_connect(branch.name, branch.name);

    perform from public.dblink(
        branch.name,
        format(
            $sql$
